...
describe('trader/rule/index', function () {
  ...
  describe('judgeForPurchase', () => {
    describe('when base is BTC', () => {
      it('when last ticker ask rate < 93% of min rate asset, return BTC 0.1units and ask * 1.02', () => {
        let result = rule.judgeForPurchase('BTC', 'ETH', [{ ask: 92 }], [{ rate: 100 }]);
        expect(result.units).to.equal(Math.trunc(100000000 * 0.1 / (92 * 1.02)) / 100000000);
      ...
      it('when assets are empty, should return BTC 0.1units units and ask * 1.02', () => {
