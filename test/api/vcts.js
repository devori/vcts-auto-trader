...
describe('api/vcts', function () {
    ...
    describe('getExchangeInfo', () => {
        before(() => {
            nock(env.VCTS_BASE_URL)
                .get(`/api/v1/public/markets/${MARKET}/exchange-info`)
                .reply(200, {
                    'BTC': {
                        'ETH': {
                            rate: {
        ...
        it('should return exchangeInfo when it calls', done => {
            vctsApi.getExchangeInfo(MARKET).then(exchangeInfo => {
                expect(exchangeInfo.BTC.ETH.rate.step).to.equal(1);
                expect(exchangeInfo.BTC.ETH.units.step).to.equal(2);
                done();
            })
        });
    });
    ...
    describe('buy', () => {
        ...
        it('should call url using post to buy when it called', done => {
            vctsApi.buy(USERNAME, MARKET, 'BTC', 'ETH', 1, 0.1).then(res => {
    ...
    describe('sell', () => {
        ...
        it('should call url using delete to sell when it called', done => {
            vctsApi.sell(USERNAME, MARKET, 'BTC', 'ETH', 1, 0.1).then(res => {
